...
def run_constrained_allocation(
    ...
    # Check if there are applications already submitted
    app_lookup = {}
    ...
    # 1. Explicitly applied candidates prioritized slightly or fair competition based on score
    ...
    # Max candidates per state per internship to ensure geographic diversity (PM Scheme constraint)
    # If regional diversity is enabled, cap single state dominance at 50% of vacancies (if vacancies > 2)
    for pair in candidate_pairs:
        ...
        # Skip if internship has no slots left
        if len(internship_allocations[i_id]) >= internship_slots[i_id]:
            continue

        # Check regional constraint if enabled
