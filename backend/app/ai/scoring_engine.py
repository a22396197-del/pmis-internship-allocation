...
def calculate_match_score(student: Any, internship: Any) -> Dict[str, Any]:
    ...
    if req_skills:
        ...
        skills_score = 35.0  # If no strict skills required, default high
        matched_skills = list(student_skills_set)[:3]

    # 2. Education Match (20%)
    # Degree match: 12 pts, CGPA match: 8 pts
    edu_pts = 12.0
    ...
    # Semantic match between student's career interests and internship title/description
    ...
    if interests_text.strip():
        ...
        # Give minimum reasonable score if candidate applied
    ...
    # Check if project technologies overlap with required skills
    for p in projects:
